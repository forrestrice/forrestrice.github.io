...
Bridgebots currently supports most of these fields using the `DealRecord`, `BoardRecord`, `Commentary`, `BidMetadata` classes.
...
from bridgebots.board_record import BidMetadata, Commentary, BoardRecord, DealRecord
...
Both of these are inputs into `BoardRecord` - the record of the play of a deal at a single table.
...
board_records=[BoardRecord(
	...
	names={NORTH: 'smalark', SOUTH: 'PrinceBen', EAST: 'granola357', WEST: 'Forrest_'},
	date=2020-03-13, event=QuickTricks Club Game,
...
All of these classes are primarily data holders, so they don't have many utility methods other than `__eq__` and `__repr__`.
...
47
```
Or maybe I'm interested in going over every slam in the tournament.
```python
slam_deal_dict = defaultdict(list)
for deal_record in results:
    for board_record in deal_record.board_records:
        if "6" in board_record.contract or "7" in board_record.contract:
